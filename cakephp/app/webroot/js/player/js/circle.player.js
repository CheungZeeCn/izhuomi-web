...
CirclePlayer.prototype = {
	...
	_initPlayer: function() {
		...
		this.player.bind($.jPlayer.event.timeupdate + this.eventNamespace, function(event) {
			if (!self.dragging) {
                //console.log(" debug  is not self.dragging xx");
				self._timeupdate(event.jPlayer.status.currentPercentAbsolute);
			}
            //console.log(" debug self.dragging  " + event.jPlayer.status.currentTime);
		});
	...
	myTimeupdate: function(time) {
		var self = this;
        var percent = 100 * time / self.audio.duration;
        self.player.jPlayer("playHead", percent );
        console.log("play time" + time);
	},
	timeupdate: function(time) {
        console.log("get time in timeupdate"  + time);
        var percent = 10;
